              ...
              <>
                ...
                <div className="relative ml-2">
                  ...
                  >
                    ...
                    <div className="w-9 h-9 rounded-full bg-[#3A4D39] text-[#ECE3CE] flex items-center justify-center text-sm shadow-md border-2 border-[#ECE3CE]">
                      ...
                      <>
                        ...
                        >
                          <div className="px-4 py-3 border-b border-[#3A4D39]/10 bg-[#3A4D39]/5">
                            <p className="text-xs text-[#3A4D39]/70 font-bold uppercase">Signed in as</p>
                            <p className="text-sm font-bold text-[#3A4D39] truncate">{user.email}</p>
                          </div>
                          <Link
                            to="/settings"
                            onClick={() => setUserMenuOpen(false)}
          ...
          <>
            ...
            >
              <div className="flex justify-between items-center mb-8 border-b border-[#3A4D39]/10 pb-4">
                ...
                <button onClick={() => setMobileMenuOpen(false)} className="p-1 hover:bg-[#3A4D39]/10 rounded-full">
                  <XMarkIcon className="w-8 h-8 text-[#3A4D39]" />
                </button>
              </div>

              <div className="flex flex-col gap-4 text-center">
                {(user ? [...authLeft, ...authRight] : publicLeft).map((link) => (
                   <Link
                    key={link.name}
                    to={link.href}
                    onClick={() => setMobileMenuOpen(false)}
                   ...
                   <div className="mt-4 pt-6 border-t border-[#3A4D39]/10 w-full">
                      <div className="flex items-center justify-center gap-3 mb-6">
                         ...
                         <div className="text-left">
                            <p className="text-sm font-bold text-[#3A4D39]">{user.first_name} {user.last_name}</p>
                            <p className="text-xs text-[#3A4D39]/70">{user.email}</p>
                         </div>
                      </div>
                      <Link
                        to="/settings"
...
const NavLink = ({ to, children }) => (
  ...
  >
    {children}
    <span className="absolute -bottom-1 left-0 w-0 h-[2px] bg-[#3A4D39] transition-all duration-300 group-hover:w-full opacity-0 group-hover:opacity-100" />
  </Link>
);